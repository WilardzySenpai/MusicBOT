...
const { getStations } = require('radio-browser')
  ...
  run: async(client,message,args) => {
    ...
    getStations({
			...
			.then(async data => {
				if (!data[0]) return message.channel.send('<a:animated_cross:881469660250394675>No radio found with that name');
				...
				try {
					...
					return message.reply('<a:animated_cross:881469660250394675>You didn\'t choose a song in time.');
				}
				...
				if (index < 0 || index > max - 1) return message.reply(`<a:animated_cross:881469660250394675>The number you provided was too small or too big (1-${max}).`);
