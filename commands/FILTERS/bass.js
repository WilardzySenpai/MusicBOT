...
const player = message.client.manager.players.get(message.guild.id)
		...
		player.clearEffects()
		const msg = await message.channel.send(`<a:Loading_Color:916880909452599336>Turning off **bass**.`);
 ...
 const m1 = await message.channel.send("<a:Loading_Color:916880909452599336>Turning on **bass**.");
	// Change bassboost value
	player.setBassboost(!player.bassboost)
	
	 const bass = new MessageEmbed()
            .setTitle("<a:SA_green_check:881469557657710592> Turned on: bass",)
            .setColor('#000001');
