...
if (!player.queue.current.isSeekable) {
		...
		try {
			...
			message.channel.send({embed : {
			  color: "BLACK",
			  description:`<a:SA_green_check:881469557657710592> Speed is ${player.speed}`}});
		} catch (err) {
  	...
  	if (args[0].toLowerCase() == 'reset' || args[0].toLowerCase() == 'off') {
		player.clearEffects()
		const msg = await message.channel.send(`<a:Loading_Color:916880909452599336>Turning off **speed**.`);
