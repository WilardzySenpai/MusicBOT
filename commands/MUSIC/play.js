...
let msg = message.channel.send(`searching  for \`${search}\``).then(m => m.delete({ timeout: 5000 }));
  ...
  try {
    ...
    let embed = new MessageEmbed()
      ...
      .setDescription(`<a:animated_cross:881469660250394675> i can't find result about \`${search}\``
       )
    return message.channel.send(embed)
  }
  ...
  switch (res.loadType) {
    case 'NO_MATCHES':
      ...
      let embed = new MessageEmbed()
        ...
        .setDescription(`<a:animated_cross:881469660250394675> i can't find results about \`${search}\``)
      return message.channel.send(embed)
